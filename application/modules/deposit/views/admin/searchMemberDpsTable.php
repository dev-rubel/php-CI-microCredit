<?php
	if(!empty($singleMemberDps)){
		foreach($singleMemberDps as $k=> $each){
			$year = explode('-',$each['dpsDate']);
			$year = current($year);
			$sortData[$year][] = $each;
		}
	}
...
<?php if(!empty($singleMemberDps)): foreach($sortData as $key=>$each): 
				...
				<div id="collapse<?php echo $key;?>" class="panel-collapse collapse">
					<div class="panel-body">
						<table class="table table-bordered">
							...
							<tbody>
							<?php $currentMonth = explode('-',$each[0]['dpsDate']);
							...
							<?php foreach($each as $k=>$each2): 
								...
								$currentMonth2 = explode('-',$each2['dpsDate']);
								$currentMonth2 = $currentMonth2[1];

								if($each2['dr_cr']=='CR'){
									$runningBalance += $each2['dpsAmount'];
									$yearBalance += $each2['dpsAmount'];
								} else { // dr debit
									$runningBalance -= $each2['dpsAmount'];
									$yearBalance -= $each2['dpsAmount'];
								}
								...
								<tr>
									<td><?php echo $k+1;?></td>
									<td><?php echo $each2['dpsDate'];?></td>
									<td><?php echo $each2['dr_cr']=='DR'?$each2['dpsAmount']:'';?></td>
									<td><?php echo $each2['dr_cr']=='CR'?$each2['dpsAmount']:'';?></td>
